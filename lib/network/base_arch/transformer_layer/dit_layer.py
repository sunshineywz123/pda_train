...
from lib.network.base_arch.transformer_layer.sd_layer import FeedForward, modulate, BLCGroupNorm, GN_GROUPS
...
class BasicDiTTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # self.img_block = ResBlock(d_model, d_model, dropout)
    ...
    def forward(
        ...
        x = tgt
        # x = self.img_block(x, f_imgseq, timestep)
        (
            shift_sa,
            scale_sa,
            gate_sa,
...
class ResBlock(nn.Module):
    def __init__(self, channels, emb_channels, dropout):
        ...
        self.norm = BLCGroupNorm(GN_GROUPS, channels)
        self.in_layer = nn.Sequential(
            nn.SiLU(),
            nn.Linear(channels, channels),
        )
        emb_out_channels = 3 * channels
    ...
    def reset_parameters(self):
        nn.init.constant_(self.emb_layer[-1].weight, 0)
        nn.init.constant_(self.emb_layer[-1].bias, 0)

    def forward(self, x, f_imgseq, emb):
        emb_out = self.emb_layer(emb)
        gate, scale, shift = emb_out.chunk(3, dim=-1)
        h = self.in_layer(modulate(self.norm(f_imgseq), shift, scale))
        h = gate * h
        return x + h
