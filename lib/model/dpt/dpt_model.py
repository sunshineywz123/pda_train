...
class DptDepthModel(pl.LightningModule):
    def __init__(
        self,
        pipeline,
        optimizer=None,
    ...
    def compute_metrics(self, batch, outputs):
        ...
        # 实际上 midas可以直接输出metric depth, 但为了和marigold比较公平    
        metric_depth = recover_metric_depth(pred_dpt, gt_dpt, mask0=~mask_invalid)
        # gt_dpt = 1 / gt_dpt
        # metric_depth = 1 / metric_depth
        # metric_depth = pred_dpt
    ...
    def configure_optimizers(self):
        ...
        for k, v in self.pipeline.named_parameters():
            if v.requires_grad:
                if 'output_conv' in k:
                    params_head.append(v)
                else:
                    params_backbone.append(v)
        params = [{'params': params_head, 'lr': 1e-4, 'name': 'head'},
                  {'params': params_backbone, 'lr': 1e-5, 'name': 'backbone'}]
