        ...
        for i, (resnet, attn, motion_module) in enumerate(blocks):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = motion_module(
                    ...
                    temb, 
                    encoder_hidden_states=encoder_hidden_states,
                    batch_size=batch_size) if motion_module is not None else hidden_states
...
class DownBlock2D(nn.Module):
    ...
    def forward(
        self, hidden_states: torch.FloatTensor, temb: Optional[torch.FloatTensor] = None, scale: float = 1.0, batch_size: int = 1,
    ) -> Tuple[torch.FloatTensor, Tuple[torch.FloatTensor, ...]]:
        output_states = ()
        ...
        for resnet, motion_module in zip(self.resnets, self.motion_modules):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = motion_module(hidden_states, temb, batch_size=batch_size) if motion_module is not None else hidden_states
