...
class DiffusionDepth(nn.Module, PipelineHelper):
    def __init__(self, 
                 scheduler_opt_train,
                 scheduler_opt_test,
                 unet_opt,
        ...
        self.unet = UNet2DConditionModel(**unet_opt)
        ...
        self._test_diffusion_steps = test_diffusion_steps
    ...
    def normalized_depth(self, dpt, mask=None):
        if mask is None:
            dpt_min = dpt.min(-1, keepdim=True)[0].min(-2, keepdim=True)[0]
            dpt_max = dpt.max(-1, keepdim=True)[0].max(-2, keepdim=True)[0]
            msk = (dpt_max - dpt_min) < EPS
            if msk.any(): dpt_max[msk] = dpt_min[msk] + EPS
        else:
            dpt_min_list, dpt_max_list = [], []
            for b in range(len(dpt)):
                msk = mask[b]
                dpt_min = dpt[b][msk].min()
                dpt_max = dpt[b][msk].max()
                dpt_min_list = dpt_min_list + [dpt_min]
                dpt_max_list = dpt_max_list + [dpt_max]
                if (dpt_max - dpt_min) < EPS:
                    dpt_max = dpt_min + EPS
            dpt_min = torch.tensor(dpt_min_list, dtype=dpt.dtype, device=dpt.device)[:, None, None, None]
            dpt_max = torch.tensor(dpt_max_list, dtype=dpt.dtype, device=dpt.device)[:, None, None, None]
    ...
    def forward_train_batch(self, batch):
        ...
        depth, dpt_min, dpt_max = self.normalized_depth(batch['depth'], batch['mask']>0.5)
        cond = batch['image']
        tar_latent = depth
        B = len(tar_latent)
        # add noise to gt
        ...
        loss = F.mse_loss(target_pred[batch['mask']>0.5], target[batch['mask']>0.5], reduction="mean")
        outputs["loss"] = loss
        return outputs
    ...
    def forward_test(self, batch, num_inference_steps=50):
        ...
        scheduler.set_timesteps(self._test_diffusion_steps, device=cond.device)
        timesteps = scheduler.timesteps  # [T]

        x_latent = torch.randn(cond[:, :1].shape, device=cond.device, dtype=cond.dtype)  
