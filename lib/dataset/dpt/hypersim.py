...
class Dataset(BaseDataset):
    def build_metas(self):
        ...
        # hold_out = self.cfg.holdout
        # test_scenes = scenes[0:len(scenes):hold_out]
        # train_scenes = [scene for scene in scenes if scene not in test_scenes]
        # scenes = train_scenes if self.cfg.split == 'train' else test_scenes
        # self.rgb_files = []
        # self.dpt_files = []
        # for scene in tqdm(scenes, desc='Scanning directories of hypersim'):
            # tar_dirs = sorted(glob.glob(os.path.join(self.cfg.data_root, scene, 'images/scene_cam_*_final_preview')))
            # for tar_dir in tar_dirs:
                # frames = sorted(glob.glob(os.path.join(tar_dir, 'frame.*.color.jpg')))
                ...
                # self.rgb_files += frames
                # self.dpt_files += [frame.replace('final_preview', 'geometry_hdf5').replace('.color.jpg', '.depth_meters_plane.npz') for frame in frames]
