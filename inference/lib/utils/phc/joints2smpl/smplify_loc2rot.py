import os
import torch
from lib.utils.phc.joints2smpl.src import config
import smplx
import h5py
from lib.utils.phc.joints2smpl.src.smplify import SMPLify3D
import lib.utils.phc.joints2smpl.rotation_conversions as geometry
import argparse
...
class joints2smpl:
    ...
    def joint2smpl_ori(self, input_joints, init_params=None):
        _smplify = self.smplify  # if init_params is None else self.smplify_fast
        pred_pose = torch.zeros(self.batch_size, 72).to(self.device)
        pred_betas = torch.zeros(self.batch_size, 10).to(self.device)
        pred_cam_t = torch.zeros(self.batch_size, 3).to(self.device)
