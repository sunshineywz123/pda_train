...
def parse_args():
    ...
    parser.add_argument('--default_yaml', type=str,
                        default='/mnt/bn/haotongdata/home/linhaotong/projects/pl_htcode/configs/mlx_configs/default.yaml')
...
def main(args):
    ...
    mlx_config['arnold']['roleconfig'][0]['gpu'] = args.num_gpus
    mlx_config['arnold']['roleconfig'][0]['cpu'] = args.num_gpus * args.num_cpus
    mlx_config['arnold']['roleconfig'][0]['mem'] = args.num_gpus * args.num_mem
    mlx_config['arnold']['roleconfig'][0]['num'] = args.num_nodes

    # 把remainder args作为command参数加到mlx_config['arnold']['entrypoint']中
    mlx_config['arnold']['entrypoint'] = args.default_prefix_command + ' '.join(args.opts)
    ...
    try:
        ...
        os.system(f'mlx job submit --name {job_name} --yaml {output_path}')
...
mlx_submit() {
  if [[ -e scripts/mlx_submit/run.py ]]; then
    python3 scripts/mlx_submit/run.py "$@"
  else
