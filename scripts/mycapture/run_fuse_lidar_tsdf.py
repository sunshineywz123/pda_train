...
def main(args):
    ...
    for idx, name in tqdm(enumerate(names)):
        ...
        color = np.asarray(imageio.imread(color_path)).astype(np.uint8)
        ...
        rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
            color, depth, depth_trunc=4.0, convert_rgb_to_intensity=False, depth_scale=1.,
        )
        volume.integrate(
            rgbd,
    ...
    output_ply_path = join(args.input_path, args.scene, 'points_lidar_tsdf.ply')
