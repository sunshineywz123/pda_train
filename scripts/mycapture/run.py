...
def parse_args():
    ...
    parser.add_argument('--input_data_dir', type=str, default='/mnt/bn/haotongdata/Datasets/mycapture_stray_scanner')
    parser.add_argument('--scene', type=str, default='human_31f344e86e')
    parser.add_argument('--rot', type=int, default=0)
    parser.add_argument('--top_percent', type=int, default=15)
    parser.add_argument('--max_interval', type=int, default=30)
...
def main(args):
    ...
    if not os.path.exists(join(data_root, 'rgb')):
        if args.rot > 0:
            cmd = 'ffmpeg  -i {} -vf "rotate=PI" {}'.format(join(data_root, 'rgb.mp4'), join(data_root, 'rgb_rot.mp4'))
            os.system(cmd)
            os.makedirs(join(data_root, 'rgb'), exist_ok=True)
            os.system(f'ffmpeg -i {join(data_root, "rgb_rot.mp4")} -start_number 0 -q:v 1 {join(data_root, "rgb")}/%06d.jpg')
            for frame_id in tqdm(range(len(os.listdir(join(data_root, 'depth'))))):
                depth = imageio.imread(join(data_root, 'depth', '{:06d}.png'.format(frame_id)))
                depth = cv2.rotate(depth, cv2.ROTATE_180)
                imageio.imwrite(join(data_root, 'depth', '{:06d}.png'.format(frame_id)), depth)

                conf = imageio.imread(join(data_root, 'confidence', '{:06d}.png'.format(frame_id)))
                conf = cv2.rotate(conf, cv2.ROTATE_180)
                imageio.imwrite(join(data_root, 'confidence', '{:06d}.png'.format(frame_id)), conf)
        else:
            os.makedirs(join(data_root, 'rgb'), exist_ok=True)
            os.system(f'ffmpeg -i {join(data_root, "rgb.mp4")} -start_number 0 -q:v 1 {join(data_root, "rgb")}/%06d.jpg')
    if 'static_camera' not in args.scene:
