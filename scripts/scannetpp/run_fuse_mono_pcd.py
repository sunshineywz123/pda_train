...
from trdparties.colmap.read_write_model import read_cameras_binary, read_images_binary, read_model, rotmat2qvec
...
def fps(c2ws, num):
    ...
    selected_indices = []
    distances_pos = np.full(len(pos_all), np.inf)
    distances_quan = np.full(len(quan_all), np.inf)
    selected_indices.append(0)
    last_selected_type = 'pos'  # Start with 'pos' type
    
    for idx in range(1, num):
        if idx % 2 == 1:  # Odd index, consider position
            dist_to_last_selected = np.linalg.norm(pos_all - pos_all[selected_indices[-1]], axis=1)
            distances_pos = np.minimum(distances_pos, dist_to_last_selected)
            next_selected = np.argmax(distances_pos)
        else:  # Even index, consider quaternion
            # Calculate distances to all points from all selected points based on quaternion
            for selected_idx in selected_indices:
                dist_to_selected = np.linalg.norm(quan_all - quan_all[selected_idx], axis=1)
                distances_quan = np.minimum(distances_quan, dist_to_selected)
            next_selected = np.argmax(distances_quan)
        selected_indices.append(next_selected)
        # Update the type for the next iteration
        last_selected_type = 'quan' if last_selected_type == 'pos' else 'pos'
    return selected_indices

def main(args):
    # cams, images, points = read_model(args.colmap_path)
    colmap_path = join(args.input_path, 'colmap/sparse/0')
    image_path = join(args.input_path, 'images')
    depth_path = join(args.workspace_path, args.exp_name, 'results', args.output_tag, 'orig_pred')
    output_ply_path = join(args.workspace_path, args.exp_name, 'results', args.output_tag, 'plys/fusion.ply')
    ...
    info = json.load(open(join(args.input_path, 'depth/v1/info.json'), 'r'))
    for idx, name in tqdm(enumerate(names)):
        ...
        msk = np.logical_and(depth > (args.min_depth / info['colmap2metric_from_zipnerf']), depth < (args.max_depth/info['colmap2metric_from_zipnerf']))
    ...
    # output_ply_path = join(args.input_path, 'depth/v1/pcd/points.ply')
    os.makedirs(os.path.dirname(output_ply_path), exist_ok=True)
...
def parse_args():
    ...
    parser.add_argument('--workspace_path', type=str, default='/mnt/bn/haotongdata/home/linhaotong/projects/pl_htcode/data/pl_htcode/outputs/depth_estimation')
    parser.add_argument('--input_path', type=str, default='/mnt/bn/haotongdata/Datasets/scannetpp/data/5f99900f09/merge_dslr_iphone')
    parser.add_argument('--exp_name', type=str, default='june_depthanythingmetric_scannetpp_hypersim')
    parser.add_argument('--output_tag', type=str, default='5f99900f09')
