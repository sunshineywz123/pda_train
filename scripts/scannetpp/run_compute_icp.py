...
def evalute_ply_path(source_ply_path, target_ply_path):
...
def main(args, exp, tag, scene):
    source_ply_path  = join(args.input_path, f'plys/zipnerf_tsdf.ply')
    target_ply_path = join(args.input_path.replace('merge_dslr_iphone', 'scans'), f'mesh_aligned_0.05.ply')
    metrics = evalute_ply_path(source_ply_path, target_ply_path)
    return metrics
    Log.info(scene)
    Log.info(metrics)
    source_ply_path = join(args.input_path, f'plys/lidar_tsdf.ply')
    metrics = evalute_ply_path(source_ply_path, target_ply_path)
    Log.info(metrics)
...
if __name__ == '__main__':
    ...
    for scene in scenes:
        ...
        # exp = 'june_depthanythingmetric_scannetpp_0614_hypersim'
        exp = 'june_depthanythingmetric_scannetpp_0614_hypersim_mask_far_orig_depth_anything_v2'
        # exp = 'june_depthanythingmetric_scannetpp_0614_hypersim_mask_far'
        depth_path_dir = f'/mnt/bn/haotongdata/home/linhaotong/workspaces/pl_htcode/outputs/depth_estimation/{exp}/results/{scene}_0703/orig_pred'
        tag = f'{scene}_0703'
        args.input_path = input_path
        transformation = main(args, exp, tag, scene)
        print(scene, transformation)
        np.savetxt(join(input_path, f'plys/transformation.txt'), transformation)