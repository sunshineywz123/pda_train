...
def detect_blurry_frames(input_dir, output_clear_dir, top_percent=30, max_interval=120, min_interval=6):
    ...
    num_to_select = max(1, len(sharpness_values) // top_percent)
    selected_frames = []
    ...
    #             break
    for i in tqdm(range(len(sharpness_values)), desc='Selecting sharp frames'):
        frame_name = sharpness_values[i][0]
        frame_index = image_files.index(frame_name)
        if all(abs(frame_index - image_files.index(idx)) >= min_interval for idx in selected_frames):
...
def parse_args():
    ...
    parser.add_argument('--input_dir', type=str, default='/mnt/bn/haotongdata/Datasets/scannetpp/data/98b4ec142f/dslr/resized_images')
    parser.add_argument('--output_dir', type=str, default='/mnt/bn/haotongdata/Datasets/scannetpp/data/98b4ec142f/merge_dslr_iphone/images/dslr')
...
def main(args):
    ...
    if args.scene is not None:
        input_dir = input_dir.replace('98b4ec142f', args.scene)
        output_dir = output_dir.replace('98b4ec142f', args.scene)
    input_mask_dir = input_dir.replace('resized_images', 'resized_anon_masks')
    output_mask_dir = output_dir.replace('images', 'masks')
    ...
    np.random.seed(42)
    num_select = 1000
    if len(files) > (num_select + 50):
        files = np.random.choice(files, num_select, replace=False).tolist()
    for file in tqdm(files):
        src_file = join(input_dir, file)
        tar_file = join(output_dir, file)
        os.system('ln -s {} {}'.format(src_file, tar_file))
        src_file = join(input_mask_dir, file.replace('.JPG', '.png') if '.JPG' in file else file.replace('.jpg', '.png'))
        tar_file = join(output_mask_dir, file + '.png')
        os.system('ln -s {} {}'.format(src_file, tar_file))
