...
def get_metadata(scene):
    ...
    depth_files = sorted(glob.glob(join('/mnt/bn/haotongdata/Datasets/scannetpp/data', scene, 'iphone', 'render_depth/*.png')))
    if len(depth_files) == 0:
        depth_files = sorted(glob.glob(join('/mnt/bn/haotongdata/Datasets/scannetpp/data', scene, 'iphone', f'render_depth/{scene}/*.png')))
        rgb_files = [depth_file.replace(f'iphone/render_depth/{scene}', 'iphone/rgb').replace('.png', '.jpg') for depth_file in depth_files]
        low_res_files = [rgb_file.replace('iphone/rgb', 'iphone/depth').replace('.jpg', '.png') for rgb_file in rgb_files]
    else:
        rgb_files = [depth_file.replace('iphone/render_depth', 'iphone/rgb').replace('.png', '.jpg') for depth_file in depth_files]
        low_res_files = [rgb_file.replace('iphone/rgb', 'iphone/depth').replace('.jpg', '.png') for rgb_file in rgb_files]
    try:
