...
from depth_fusion import fuse_points
...
def read_ixts_exts_sdpt(cams, images, points, img_names=None):
    ...
    if img_names is None:
        ...
        imageids = [name2imageid[name] for name in img_names]
    # ixts, exts, sdpt
...
def read_imgs(input_dir):
    ...
    return (np.asarray(imgs) / 255.).astype(np.float32), img_names

def main(args):
    cams, images, points = read_model(args.colmap_input)
    imgs, img_names = read_imgs(args.rgb_input)
    ixts, exts, sdpts, hws = read_ixts_exts_sdpt(cams, images, points, img_names)
    ...
    rgbs = torch.tensor(imgs).cuda()
    dpts = torch.tensor(aligned_dpts).cuda()
    hws = torch.tensor(hws).cuda()
     
    dpt_h, dpt_w = dpt.shape
    h, w = hws[0]
    ixts[:, :1] = ixts[:, :1] * dpt_w / w
    ixts[:, 1:2] = ixts[:, 1:2] * dpt_h / h
    hws[:, 0] = dpt_h
    hws[:, 1] = dpt_w
    xyzs, rgbs = fuse_points(
        exts,
        ixts,
        rgbs,
        dpts,
    ...
    pcd.points = o3d.utility.Vector3dVector(torch.cat(xyzs, dim=-2).detach().cpu().numpy())
    pcd.colors = o3d.utility.Vector3dVector(torch.cat(rgbs, dim=-2).detach().cpu().numpy())
    voxel_size = 0.1
    pcd = pcd.voxel_down_sample(voxel_size)
    os.makedirs(args.output, exist_ok=True)
    o3d.io.write_point_cloud(join(args.output, 'fusible.ply'), pcd)
    # os.makedirs(args.output, exist_ok=True)
