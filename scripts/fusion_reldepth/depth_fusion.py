...
import torch
sys.path.append('.')
sys.path.append('.trdparties/easyvolcap')
from tqdm import tqdm
# from trdparties.easyvolcap.easyvolcap.utils.console_utils import *
from trdparties.easyvolcap.easyvolcap.utils.ray_utils import get_rays
from trdparties.easyvolcap.easyvolcap.utils.easy_utils import read_camera
from trdparties.easyvolcap.easyvolcap.utils.cam_utils import compute_camera_similarity
from trdparties.easyvolcap.easyvolcap.utils.image_utils import resize_image, pad_image
from trdparties.easyvolcap.easyvolcap.utils.chunk_utils import multi_gather, multi_scatter
from trdparties.easyvolcap.easyvolcap.utils.math_utils import affine_padding, affine_inverse
from trdparties.easyvolcap.easyvolcap.utils.data_utils import to_cuda, to_tensor, to_x, add_batch, load_image, load_depth, export_pts, read_pfm
from trdparties.easyvolcap.easyvolcap.utils.fusion_utils import filter_global_points, depth_geometry_consistency, compute_consistency
...
def fuse_points(
    ...
    cens = []
    dirs = []
    for v in tqdm(range(len(w2cs)), desc='Loading depths & images & rays'):
        H, W = hws[v]
        ray_o, ray_d = get_rays(H, W, ixts[v], w2cs[v][:3, :3], w2cs[v][:3, 3:], z_depth=True)
        cens.append(ray_o)
        dirs.append(ray_d)
    
    c2ws = affine_inverse(w2cs)
    _, src_inds = compute_camera_similarity(c2ws, c2ws) 
    xyzs_out, rgbs_out = [], []
    for v in tqdm(range(len(w2cs)), desc='Fusing depths'):
        # Prepare source views' information
        src_ind = src_inds[v, 1:1 + n_srcs]  # 4,
        dpt_src = dpts[src_ind]  # 4, HW
        ixt_src = ixts[src_ind]  # 4, 3, 3
