...
def parse_args():
    ...
    parser.add_argument('--semantic_input', type=str)
...
def recover_align_dpt(sdpt, dpt, hw, roimsk, use_disp):
    err = sdpt[:, 3]
...
def read_segs(input_dir):
    img_names = sorted(os.listdir(join(input_dir)))
    img_paths = [join(input_dir, img_name) for img_name in img_names]
    imgs = [] 
    for img_path in tqdm(img_paths, desc='reading segs'):
        imgs.append(np.load(img_path)['arr_0'])
...
def main(args):
    ...
    segs = read_segs(args.semantic_input)
    aligned_dpts = []
    for idx in tqdm(range(len(monodpts))):
        dpt = monodpts[idx]
        aligned_dpt = recover_align_dpt(sdpts[idx], dpt, hws[idx], segs[idx]==6, use_disp=args.use_disp)
    ...
    for i in tqdm(range(0, len(aligned_dpts), args.step)):
        ...
        mask[1600:] = False
        mask[monodpts[i]<=0.02] = False
        
        points = unproject(dpt, ixt, ext)
        points = points[mask]
        colors = imgs[i][mask].reshape(-1, 3)
        
        # points_msk = points[:, 1] < -1.
        points_msk = (points[:, 1] < 0.5) & (points[:, 1] > 0.)
        points = points[points_msk]
        colors = colors[points_msk]
    ...
    voxel_size = 0.01
    pcd = pcd.voxel_down_sample(voxel_size)
    o3d.io.write_point_cloud(join(args.output, 'output.ply'), pcd)
