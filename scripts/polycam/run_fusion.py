...
def read_ixt(cam):
    ixt = np.eye(3)
    ixt[0, 0], ixt[1, 1], ixt[0, 2], ixt[1, 2] = cam['fx'], cam['fy'], cam['cx'], cam['cy']
    return ixt

def read_ext(cam):
    ext = np.eye(4)
    ext[0, 0], ext[0, 1], ext[0, 2], ext[0, 3] = cam['t_00'], cam['t_01'], cam['t_02'], cam['t_03']
    ext[1, 0], ext[1, 1], ext[1, 2], ext[1, 3] = cam['t_10'], cam['t_11'], cam['t_12'], cam['t_13']
    ext[2, 0], ext[2, 1], ext[2, 2], ext[2, 3] = cam['t_20'], cam['t_21'], cam['t_22'], cam['t_23']
    return ext

def main(args):
    # cams, images, points = read_model(args.colmap_path)
    image_names = sorted(os.listdir(join(args.input_path, 'images')))
    pcds = []
    pcd_colors = []
    for image_name in tqdm(image_names):
        cam_json = join(args.input_path, 'cameras', image_name[:-4] + '.json')
        depth_path = join(args.input_path, 'depth', image_name[:-4] + '.png')
        conf_path = join(args.input_path, 'confidence', image_name[:-4] + '.png')
        
        cam = json.load(open(cam_json))
        ixt, ext = read_ixt(cam), read_ext(cam)
        
        depth = (imageio.imread(depth_path)).astype(np.float32) / 1000.
        conf = np.asarray(cv2.imread(conf_path, cv2.IMREAD_ANYDEPTH))
        img = np.asarray(imageio.imread(join(args.input_path, 'images', image_name))) / 255.
        msk = conf==255
        ...
        c2w = ext
        rays_o, rays_d = gen_rays(c2w, ixt, depth.shape, 0., 0., 0., 0.)
        
        pcd = rays_o + rays_d * depth[..., None]
        pcd_color = img
        
        pcds.append(pcd[msk].reshape(-1, 3))
        pcd_colors.append(pcd_color[msk].reshape(-1, 3))
    o3d_pcd = o3d.geometry.PointCloud()
    o3d_pcd.points = o3d.utility.Vector3dVector(np.concatenate(pcds, 0).reshape(-1, 3))
    o3d_pcd.colors = o3d.utility.Vector3dVector(np.concatenate(pcd_colors, 0).reshape(-1, 3))
    output_ply_path = join(args.output_dir_path, args.output_ply_name)
...
def parse_args():
    ...
    parser.add_argument('--input_path', type=str, default='/mnt/bn/haotongdata/Datasets/polycam/mmw_meetingroom/keyframes')
    parser.add_argument('--output_dir_path', type=str, default='/mnt/bn/haotongdata/home/linhaotong/projects/pl_htcode/data/pl_htcode/plys')
    parser.add_argument('--output_ply_name', type=str, default='mmw_meetingroom.ply')
